...
function handleNavigateCloseMobileOverlay() {
  layoutShell.closeSidebarMobile();
}
</script>
...
<template>
  <aside
    ...
    :class="[
      layoutShell.isSidebarCollapsed ? 'lg:w-20' : 'lg:w-64',
      layoutShell.isSidebarMobileOpen
        ? 'translate-x-0'
  ...
  >
    ...
    <div
      ...
      :class="{ 'lg:px-2': layoutShell.isSidebarCollapsed }"
    >
      <div
      ...
      <RouterLink
        :to="{ name: 'dashboard' }"
        class="relative flex min-h-0 min-w-0 flex-1 items-center gap-2.5 rounded-lg outline-none ring-primary/25 transition hover:bg-white/40 focus-visible:ring-2 sm:gap-3"
        :class="{ 'lg:justify-center': layoutShell.isSidebarCollapsed }"
        aria-label="대시보드 홈"
        @click="handleNavigateCloseMobileOverlay"
      >
        <div
        ...
        <div
          ...
          :class="{ 'lg:hidden': layoutShell.isSidebarCollapsed }"
        >
      ...
      <template v-for="(item, i) in menu" :key="`${item.type}-${i}`">
        ...
        <RouterLink
          ...
          :class="[
            ...
            layoutShell.isSidebarCollapsed ? 'lg:justify-center lg:px-2' : '',
          ]"
          :title="layoutShell.isSidebarCollapsed ? item.label : undefined"
          @click="handleNavigateCloseMobileOverlay"
        >
          <span
          ...
          <span
            class="truncate"
            :class="{ 'lg:sr-only': layoutShell.isSidebarCollapsed }"
          >{{ item.label }}</span>
        </RouterLink>
      </template>
    </nav>
