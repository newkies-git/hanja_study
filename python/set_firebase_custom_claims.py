...
사용 예:
  pip install -r python/requirements-firebase.txt
  export GOOGLE_APPLICATION_CREDENTIALS=/절대경로/서비스계정.json

  # 이메일로 admin 부여
  python python/set_firebase_custom_claims.py --project-id chusa-1817 --email you@example.com --admin true

  # uid로 admin 부여
  python python/set_firebase_custom_claims.py --project-id chusa-1817 --uid SOME_UID --admin true
...
try:
    ...
    print("firebase-admin이 필요합니다: pip install -r python/requirements-firebase.txt", file=sys.stderr)
    raise SystemExit(1)
