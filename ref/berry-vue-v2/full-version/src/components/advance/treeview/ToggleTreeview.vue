...
function selectionsInfo(path) {
  const node = path.reduce((current, i) => current.children[i], {
    children: items
  });
...
<template>
  <UiChildCard title="Toggle">
    <v-treeview v-model:selected="model" :items="items" item-value="id" select-strategy="classic" open-all selectable>
