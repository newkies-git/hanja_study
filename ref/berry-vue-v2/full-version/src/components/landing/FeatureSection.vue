    ...
    <v-container class="maxWidth">
      <v-row class="justify-center">
        <v-col md="6" cols="12" class="text-center">
          <h2 class="text-sm-display-large my-3" style="line-height: 1.2">What does Berry offer?</h2>
          <p>
        ...
        <v-col v-for="(item, index) in items" :key="index" md="4" sm="6" cols="12" data-aos="fade-up" data-aos-duration="800">
          <v-card variant="outlined" class="pa-5 bg-containerBg">
            <v-img :src="item.image" width="56" height="56" rounded="sm" :alt="item.title" class="bg-transparent" />
            <h3 class="mt-4 font-weight-medium">
              {{ item.title }}
            </h3>
            <p class="my-3 text-body-medium">
              {{ item.content }}
            </p>
          </v-card>
        </v-col>
