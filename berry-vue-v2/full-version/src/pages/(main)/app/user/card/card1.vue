  ...
  <v-row class="justify-content-end">
    <v-col cols="12">
      <UiParentCard title="Cards">
          ...
          <v-col cols="12" lg="4" xl="3" sm="6" v-for="(card, i) in filteredCards" :key="i">
            <v-card variant="outlined" class="card-hover-border bg-gray100">
              <v-card-text>
                ...
                <div class="mb-4 mt-5">
                  <h3>{{ card.name }}</h3>
                  <h6 class="text-disabled">{{ card.role }}</h6>

                  <p class="text-body-small mt-3 mb-4">
                    {{ card.about }}
                  </p>

                  <h6 class="text-disabled">Email</h6>
                  <h6>
                    {{ card.email }}
                  </h6>
                  <v-row class="mt-3">
                    <v-col cols="6">
                      <h6 class="text-disabled">Phone</h6>
                      <h6>
                        {{ card.contact }}
                      </h6>
                    </v-col>
                    <v-col cols="6">
                      <h6 class="text-disabled">Location</h6>
                      <h6>
                        {{ card.location }}
                      </h6>
                    </v-col>
        ...
        <v-row class="justify-space-between" align="center" gap="0">
          <v-col cols="12" lg="5" sm="6" class="text-start">
            <v-pagination density="compact" class="left-pagination" v-model="page1" active-color="primary" rounded="circle" :length="6">
            </v-pagination>
          </v-col>
          <v-col cols="12" lg="5" sm="6" class="text-end">
